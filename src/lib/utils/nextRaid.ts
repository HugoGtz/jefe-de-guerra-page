 ...
 *
 * Los horarios (`RaidNight`) son "hora de pared" en una zona horaria dada
 * (p. ej. Europe/Madrid). Esta zona alterna entre CET (UTC+1) y CEST (UTC+2)
 * según la estación, así que no podemos asumir un offset fijo.
 *
 * Enfoque (pragmático y SSR-safe — sin dependencias externas ni `window`):
 *  1. Para una fecha candidata (en UTC) y un horario "HH:MM" en el día d,
 *     averiguamos qué offset aplica la zona en ese momento usando
 *     `Intl.DateTimeFormat` con `timeZone`.
 *  2. Construimos el instante UTC que corresponde a esa hora de pared.
 *  3. Recorremos los próximos 8 días, generamos los candidatos de cada noche
 *     que caiga en ese día de la semana (en la zona destino) y nos quedamos
 *     con el instante futuro más cercano respecto a `now`.
 */
...
export type NextRaid = {
	...
	date: Date;
	/** Etiqueta legible en español, p. ej. "Core 1 · jueves 21:00". */
	label: string;
};
 ...
 * Positivo => la zona va por delante de UTC (p. ej. CEST = +120).
 */
function tzOffsetMinutes(date: Date, tz: string): number {
	...
	if (hour === 24) hour = 0;
	// Instante "como si la hora de pared de la zona fuese UTC".
	...
	// Diferencia entre esa lectura y el instante real => offset de la zona.
...
function wallTimeToUTC(
	...
	// Primera aproximación: tratar la hora de pared como si fuese UTC.
	let guess = new Date(Date.UTC(year, month - 1, day, hh, mm, 0));
	// Ajustar con el offset que la zona aplica en ese instante (1–2 pasadas
	// convergen salvo justo en el borde DST).
	for (let i = 0; i < 2; i++) {
 ...
 *
 * @param nights horarios semanales recurrentes
 * @param tz     zona horaria en la que se interpretan (p. ej. 'Europe/Madrid')
...
export function getNextRaid(
	...
	for (let offset = 0; offset <= 7; offset++) {
		...
		const wd = weekdayInTz(dayInstant, tz);
		const { year, month, day } = ymdInTz(dayInstant, tz);
		...
		for (const night of nights) {
			...
			const start = wallTimeToUTC(year, month, day, hh, mm, tz);
