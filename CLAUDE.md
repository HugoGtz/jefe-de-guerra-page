...
- **Styling:** Tailwind v4 (no config file; `@theme` tokens + `@layer` + `@utility` in `src/app.css`) combined with scoped `<style>` blocks. Tokenized scales exist for color, type (`--text-*`), tracking (`--tracking-*`) and breakpoints (`sm 600 / md 768 / lg 1024`) — use the token, never a raw value or arbitrary `text-[...]`. See **`DESIGN_SYSTEM.md`** for the rules: utilities for structural layout + token colors/type, `@utility label-caps` for uppercase labels, and scoped CSS only for the bespoke "look" (gradients/glows/animations/`clamp()` headings).
